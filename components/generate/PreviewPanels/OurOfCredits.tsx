/*
  Out of credits preview panel with upgrade/renew action.
*/
"use client"
...
type OutOfCreditsStateProps = {
  isFreeTier: boolean
  onUpgrade: () => void
}

export function OutOfCreditsState({ isFreeTier, onUpgrade }: OutOfCreditsStateProps) {
  return (
    <Card className="relative min-h-[460px] border-border/60 bg-card/70 p-6 shadow-[0_18px_45px_rgba(15,23,42,0.35)]">
      <div className="flex items-center justify-between">
        ...
        <div className="rounded-full border border-border/60 px-3 py-1 text-xs text-muted-foreground">Out of credits</div>
      </div>
      ...
      <div className="mt-6 flex h-[320px] items-center justify-center rounded-2xl border border-dashed border-border/60 bg-background/40">
        <div className="text-center">
          ...
          <div className="mt-4 text-lg font-semibold text-foreground">You're out of credits</div>
          <div className="mt-2 text-xs text-muted-foreground">
            Upgrade or renew to generate more videos.
          </div>
          <button
            onClick={onUpgrade}
          ...
          >
            {isFreeTier ? "Upgrade to Pro" : "Renew plan"}
          </button>
        </div>
      </div>
    </Card>
